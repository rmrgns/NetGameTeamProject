		...
		if (GetIsPlaying() == true) {
			...
			for (int i = 0; i < 5; ++i) {
				...
				if (fabsf(key2.x - GetTime()) > perfectCloseValue) {
					...
					if (((bin == false && bkeyd) && (GetNote(n).ntype == NOTE_TYPE::LongNote && isLongState[i]))) {
						isLongState[i] = false;
						note.enable = false;
						SetNote(n, note);
				...
				if (autoplaying) {
					...
					if (fabsf(note.time - GetTime()) < 0.01f) {
						if (note.ntype != NOTE_TYPE::LongNote) {
							...
							shp::vec2f efpos = GetEPos(note.rotPos, note.pos, playloc);
							float wr = playloc.getw() / 4;
							GameManager* GM = (GameManager*)gm;
							GM->AddObject((GameObject*)HeapDebugClass::HeapNew<SizeEffect>()->Init(
								SpriteData[5], efpos, shp::vec2f(wr, wr), 2, 10, 9));
...
void PlayStation::Render(HDC hdc)
{
	if (enable) {
		...
		float ww = wloc.getw() / 2 + (wloc.getw() / 4) * AnimClass::EaseIn(Impact, 5) / 2;
